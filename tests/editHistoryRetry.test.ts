...
describe("edit history retry", () => {
  ...
  it("keeps failed batches ordered and retries the combined batch once", async () => {
    vi.useFakeTimers();
    ...
    await retry.save("project", [segment("one")]);
    await retry.save("project", [segment("two")]);
  ...
  it("attempts a large normal edit before applying retry-buffer limits", async () => {
    const record = vi.fn();
    const states: string[] = [];
    ...
    await retry.save("project", [large]);
  ...
  it("stops after a bounded number of failed retries", async () => {
    vi.useFakeTimers();
  ...
  it("never retries history explicitly cleared before its timer fires", async () => {
    vi.useFakeTimers();
    const record = vi.fn(() => { throw new Error("database unavailable"); });
  ...
  it("drops a pending retry when its project was deleted", async () => {
    vi.useFakeTimers();
    const record = vi.fn(() => { throw new Error("database unavailable"); });
    const states: string[] = [];
