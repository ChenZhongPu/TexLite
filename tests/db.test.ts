...
describe("database migrations", () => {
    ...
    try {
      ...
      expect(migrated.prepare("SELECT version, name FROM texlite_schema_migrations").all())
        .toEqual([
          ...
          { version: 9, name: "nuwax_oauth_accounts_and_tokens" }
        ]);
  ...
  it("does not replay legacy tags when an existing database already has private-tag tables", () => {
    ...
    try {
      ...
      expect(database.prepare("SELECT version FROM texlite_schema_migrations").all()).toEqual([{ version: 1 }, { version: 2 }, { version: 3 }, { version: 4 }, { version: 5 }, { version: 6 }, { version: 7 }, { version: 8 }, { version: 9 }]);
  ...
  it("backfills legacy GitHub identities without changing the local account or sessions", () => {
    ...
    try {
      ...
      database.prepare("DELETE FROM texlite_schema_migrations WHERE version IN (4, 5, 6, 7, 8, 9)").run();
      database.close();
  ...
  it("makes optional emails unique and binds existing email invitations to a user", () => {
    ...
    try {
      ...
      database.prepare("DELETE FROM texlite_schema_migrations WHERE version IN (6, 7, 8, 9)").run();
      database.close();
      ...
      expect((database.prepare("PRAGMA table_info(project_invitations)").all() as Array<{ name: string; notnull: number }>)
        .find((column) => column.name === "email")?.notnull).toBe(0);
  ...
  it("records a migration only after its transaction succeeds", () => {
    ...
    try {
      ...
      try {
        expect(migrated.prepare("SELECT version FROM texlite_schema_migrations").all()).toEqual([{ version: 1 }, { version: 2 }, { version: 3 }, { version: 4 }, { version: 5 }, { version: 6 }, { version: 7 }, { version: 8 }, { version: 9 }]);
      } finally {
        migrated.close();
      }
    } finally {
  ...
  it("rejects a database created by a newer schema release", () => {
    ...
    database.exec(`
      ...
      INSERT INTO texlite_schema_migrations VALUES (10, 'future_schema', '2026-01-01T00:00:00.000Z');
    `);
    database.close();

    try {
      expect(() => openDatabase(migrationConfig(root, databasePath)))
        .toThrow(/version 10 is newer than this TexLite release/);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
